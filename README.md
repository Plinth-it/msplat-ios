...
For production, add the repository as a Swift package and pin the exact
release. Version `2.0.0` resolves an immutable, checksum-verified XCFramework:

```swift
.package(
    url: "https://github.com/Plinth-it/msplat-ios",
    exact: "2.0.0"
)
```

For local native development, run the build script above and add the nested
package at `msplat-ios/swift`. That manifest links the locally generated
`MsplatCore.xcframework` directly.
...
Package checkouts contain no benchmark datasets or Git LFS pointers and do not
require Git LFS. Building Msplat or consuming it from Plinth does not need the
datasets. Before running benchmarks or the Swift/Python dataset tests, download
the Garden fixture with [Git LFS](https://git-lfs.com/) and Python 3 installed:

```sh
bash scripts/download-datasets.sh
```

The helper downloads the 188 files (about 245 MiB) from the pinned, historical
2.1.1 dataset revision using an isolated temporary Git checkout. It copies only
the hydrated Garden directory into ignored `datasets/mipnerf360/garden`; it does
not configure LFS in the package checkout. Existing valid data is reused. CI uses
the same helper before dataset-dependent tests; native CTest fixtures do not
need it. The historical LFS objects remain available for reproducible benchmarks
and older releases.

Keep LFS rules out of tracked `.gitattributes` files. Xcode 27 checks for Git LFS
even when `.lfsconfig` excludes every dataset, and its PATH may not include
Homebrew. The fresh-package CI check rejects tracked datasets and LFS pointers,
resolves and builds a consumer with LFS deliberately unavailable, and checks that
the resource bundle contains only the three published Metal libraries.

`MsplatCore.xcframework` is a local build output. The three platform metallibs
are also regenerated by `scripts/build-xcframework.sh`, but their released
copies are committed as Swift package resources. Rebuild after any native or
Metal change — the Swift side otherwise links a stale header or shader library
without complaint.
