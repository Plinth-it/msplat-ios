...
Package checkouts leave benchmark datasets as Git LFS pointers via `.lfsconfig`;
building Msplat or consuming it from Plinth does not need them. Before running
benchmarks or the Swift/Python dataset tests, explicitly download them with
...
The empty `--exclude` overrides the default dataset exclusion for this download.
CI performs this download before dataset-dependent tests. Native CTest fixtures
do not need it.
